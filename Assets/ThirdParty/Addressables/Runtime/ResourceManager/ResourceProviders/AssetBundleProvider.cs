...
#define LXYD_CUSTOM_ENCRYPT_BUNDLE
    ...
    public class AssetBundleResource : IAssetBundleResource, IUpdateReceiver
    {
        ...
        private bool WaitForCompletionHandler()
        {
            ...
            if (m_RequestOperation is UnityWebRequestAsyncOperation op)
            {
                ...
                {
                    m_RequestOperation.completed -= WebRequestOperationCompleted_Faked;
            ...
            UnityEngine.Debug.Log("[Test] - WaitForCompletionHandler - m_Completed-->" + m_Completed);
            return m_Completed;
        }
        ...
        private void LoadLocalBundle()
        {
            ...
            {
#if LXYD_CUSTOM_ENCRYPT_BUNDLE
                var fileStream = new LXYDRC4FileStream(m_TransformedInternalId, FileMode.Open, FileAccess.Read);
                m_RequestOperation = AssetBundle.LoadFromStreamAsync(fileStream, m_Options == null ? 0 : m_Options.Crc);
#else
...
#if LXYD_CUSTOM_ENCRYPT_BUNDLE
            req.downloadHandler = new LXYDDownLoadFileHandler(
                internalId,
                Caching.currentCacheForWriting.path + "/" + m_Options.BundleName + "/" + m_Options.Hash,
        ...
        {
            UnityEngine.Debug.Log("[Test] - BeginWebRequestOperation--------->> Begin");
            m_TimeoutTimer = 0;
            ...
            {
                UnityEngine.Debug.Log("[Test] - BeginWebRequestOperation--------->> isDone");
                WebRequestOperationCompleted_Faked(m_RequestOperation);
            }
            else
            {
        ...
        private void WebRequestOperationCompleted_Faked(AsyncOperation op)
        {
            UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted_Faked-->Begin");
#if LXYD_CUSTOM_ENCRYPT_BUNDLE
            UnityWebRequestAsyncOperation remoteReq = op as UnityWebRequestAsyncOperation;
            var webReq = remoteReq?.webRequest;
            var downloadHandler = (LXYDDownLoadFileHandler)(webReq?.downloadHandler);
            if(downloadHandler == null)
            {
                UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted_Faked-->downloadHandler is null");
                return;
            }

            var reqHasError = true;
            if(webReq != null)
            {
                UnityWebRequestResult uwrResult = null;
                reqHasError = UnityWebRequestUtilities.RequestHasErrors(webReq, out uwrResult);
            }

            if(!downloadHandler.isHandleEnded && !reqHasError)
            {
                UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted_Faked-->" + (downloadHandler.requestOperation == null).ToString() + "  " + (downloadHandler.Completed == null).ToString());
                if(downloadHandler.requestOperation == null)
                {
                    downloadHandler.requestOperation = op;
                    downloadHandler.Completed = WebRequestOperationCompleted;
                }
                return;
            }

            downloadHandler.Completed = null;
#endif
            WebRequestOperationCompleted(op);
        }

        private void WebRequestOperationCompleted(AsyncOperation op)
        {
            UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted-->" + m_RequestCompletedCallbackCalled.ToString());
            if (m_RequestCompletedCallbackCalled)
                return;
            ...
            var webReq = remoteReq?.webRequest;
#if LXYD_CUSTOM_ENCRYPT_BUNDLE
            var downloadHandler = webReq?.downloadHandler as LXYDDownLoadFileHandler;
#else
            var downloadHandler = webReq?.downloadHandler as DownloadHandlerAssetBundle;
#endif
            UnityWebRequestResult uwrResult = null;
            var reqHasError = UnityWebRequestUtilities.RequestHasErrors(webReq, out uwrResult);
            if(webReq != null)
            {
                UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted-->webRequest-->" + reqHasError.ToString() + "  " + webReq.result.ToString() + "  " + webReq.responseCode);
            }
            if (webReq != null && !reqHasError)
            {
                if (!m_Completed)
                {
#if ENABLE_ADDRESSABLE_PROFILER
                    AddBundleToProfiler(Profiling.ContentStatus.Active, m_Source);
#endif
					UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted - Path-->" + Caching.currentCacheForWriting.path);
					UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted - downloadHandler.assetBundle-->" + m_Options.BundleName);
					UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted - downloadHandler.assetBundle-->" + m_Options.Hash);
                    m_AssetBundle = downloadHandler.assetBundle;
                    UnityEngine.Debug.Log("[Test] - WebRequestOperationCompleted - m_AssetBundle.name-->" + m_AssetBundle.name + "  " + m_AssetBundle.ToString());
            ...
            else
            {
...
#if LXYD_CUSTOM_ENCRYPT_BUNDLE
                downloadHandler = (LXYDDownLoadFileHandler)webReq.downloadHandler;
#else
                ...
                // {
                    ...
                    // if (uwrResult.ShouldRetryDownloadError())
                    // {
                        m_Retries++;
                        Debug.LogFormat(message);
                        BeginOperation();
