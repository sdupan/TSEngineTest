...
namespace UnityEditor.XCodeEditor
{
	public partial class XCProject : System.IDisposable
	{
		...
		{	
			...
			var compiler_flags = mod.compiler_flags;
			if(compiler_flags != null)
			{
				foreach( string flag in compiler_flags ) {
					this.AddOtherCFlags( flag );
				}
			}
			else
			{
				Debug.Log( "Adding compiler flags failed..." );
			}

			Debug.Log( "Adding linker flags..." );
			var linker_flags = mod.linker_flags;
			if(linker_flags != null)
			{
				foreach( string flag in linker_flags ) {
					this.AddOtherLinkerFlags( flag );
				}
			}
			else 
			{
				Debug.Log( "Adding linker flags failed..." );
			}
