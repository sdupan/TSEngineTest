...
namespace UnityEditor.GUI
{
    ...
    public class TSPropertiesEditor : Editor
    {
        private Object _scriptObject;
        //实例引用
        private TSProperties _instance;
        ...
        void OnEnable()
        {
            //Debug.Log("OnEnable");
            _scriptObject = null;
            _instance = target as TSProperties;
            if (_instance == null)
                return;

            //script object
            var targetName = $"/{nameof(TSProperties)}.cs";
            ...
            display.AddDrawer<StringDrawer>(typeof(TSProperties.String));
            display.AddDrawer<NumberDrawer>(typeof(TSProperties.Number));
            display.AddDrawer<BooleanDrawer>(typeof(TSProperties.Boolean));
            display.AddDrawer<ObjectDrawer>(typeof(TSProperties.Object));
            display.AddDrawer<ObjectArrayDrawer>(typeof(TSProperties.ObjectArray));
            display.AddDrawer<GameObjectArrayDrawer>(typeof(TSProperties.GameObjectArray));
        ...
        private DrawerInfo CreateDrawerInfo()
        {
            ...
            if (_instance != null)
            {
                ...
                var fields = typeof(TSProperties).GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
                foreach (var field in fields)
                {
                    Type elementType = field.FieldType.IsArray ? field.FieldType.GetElementType() : null;
                    if (elementType != null
                        && typeof(TSProperties.IPair).IsAssignableFrom(elementType)
                        && serializedObject.FindProperty(field.Name) != null
                        && DisplayUtility.CheckFields(elementType))
                    {

                        var options = elementType.GetCustomAttribute<TSProperties.OptionsAttribute>(false);
        ...
        void DisplayMenu()
        {
            ...
            if (menuFoldout)
            {
                ...
                {
                    var code = DisplayUtility.GenCode(_instance.GenPairs(), "public", true);
                ...
                {
                    var code = DisplayUtility.GenCode(_instance.GenPairs(), "private", true);
                    var editor = new TextEditor();
                    editor.text = code;
                    editor.OnFocus();
                    editor.Copy();
    ...
    public class DisplayUtility
    {
        ...
        {
            FieldInfo index, key;
            return typeof(TSProperties.IPair).IsAssignableFrom(type)
        ...
        public static string GenCode(TSProperties.ResultPair[] pairs, string declareType, bool useFullname)
        {
            var resultCode = "";
        ...
        public static void PraseCode(TSPropertiesEditor editor, string code)
        {
